...
function execute() {
	...
	*/
	...
	for (var i=0;i<l;i++) {
		...
		if (row.join("").split(" ").join("") != "") { // ignore blank lines
			var flaName = row[0];
			appendToLog("\n---------------------------------------------------------------------------\nCompiling "+flaName+"\n---------------------------------------------------------------------------");
		...
		// this check for the swf is not legitimate, as the publish settings may give a different swf name than the fla name
		//  hence it is commented out. 
		// TODO: somehow get the proper swf name from the publish settings (need to write publish settings to a file and read back
		//  in, I think) to do a legitimate check
...
function compile(p_folderURI,p_fileName,p_profile) {
	var fnAry = p_fileName.split("/");	
	var pathAry = p_folderURI.split("/");	
	while (pathAry[pathAry.length-1] == "") {
		pathAry.pop();
	}
	while (fnAry[0] == "..") {
		pathAry.pop();
	...
	var fn = pathAry.concat( fnAry).join("/");
	fl.outputPanel.clear();
	var doc = fl.openDocument(fn);
